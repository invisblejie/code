...
def get_district_url(df):
    hello = ["5214ea792d671f2d49865088c5436d15"]
    return "http://restapi.amap.com/v3/geocode/geo?address=" + quote(str(df['city']) + str(df['region_location']), safe="/:?=") +"&key=" + random.choice(hello)
...
class gaodedistrict(scrapy.Spider):
    ...
    def get_district_uid(self, response):
        ...
        try:
            if result["status"] == "1":
                lng[region_id] = result["geocodes"][0]["location"].split(',')[0]
                lat[region_id] = result["geocodes"][0]["location"].split(',')[1]
...
district_inner.to_csv('district.out', sep='|')
